...
void server_network_manager::broadcast_message(std::string &msg /* const std::vector<player *> &players,
                                               const player *exclude) {
    //rapidjson::Document* msg_json = msg.to_json();  // write to JSON format
     std::string msg_string = msg;// = json_utils::to_string(msg_json);   // convert to string
